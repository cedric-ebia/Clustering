...
```{r}
init_table<-read.delim(file="bdd_profilweb.txt",header = TRUE, sep = ";")
```
...
etude$source_web_rec <- fct_recode(etude$source_web,
               ...
               "Accès direct ou partenaire" = "Webmails",
               "Publicité Omnicanal" = "Email marketing",
...
```{r}
getColumnIndexByColname(etude,"log")
```
...
```{r}
final_table<-etude[,c(1,4,6,9,10,15:23)]
```
...
De ce fait, voici quelques préconisations que l'on pourrait faire:
  + Supprimer la variable utilisation_credit
  + Supprimer la variable région


# 5) Application de l'ACM

En premier lieu, nous commençons par créer une nouvelle table dans laquelle nous supprimons les variables utilisation_credit et région.

```{r}
getColumnIndexByColname(analyse_table,"region")
data_acm<-analyse_table[,-c(6,13)]
```
...
spl = sample.split(data_acm, SplitRatio = 0.70)
...
```{r}
mca_test<-MCA(data_acm, ncp=25,graph = FALSE)
```
...
Une autre façon de visualiser les variables les plus importantes sur cet axe (axes 3 et 4)

```{r}
disp_mca_2<-explor::MCA_var_plot(res, xax = 3, yax = 3, var_sup = FALSE, var_lab_min_contrib = 2,
    col_var = "Variable", symbol_var = NULL, size_var = "Contrib", size_range = c(46.875,
        625), labels_size = 10, point_size = 50, transitions = TRUE, labels_positions = NULL,
    xlim = c(-1.12, 4.38), ylim = c(-1.12, 4.38))
...
```{r}
disp_mca_3<-explor::MCA_var_plot(res, xax = 4, yax = 4, var_sup = FALSE, var_lab_min_contrib = 2,
    col_var = "Variable", symbol_var = NULL, size_var = "Contrib", size_range = c(46.875,
        625), labels_size = 10, point_size = 50, transitions = TRUE, labels_positions = NULL,
    xlim = c(-1.12, 4.38), ylim = c(-1.12, 4.38))
